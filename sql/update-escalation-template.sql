-- Update Action Plan Escalation Template with new fields
-- Changes:
-- 1. Added "Report Sent On" field 
-- 2. Renamed "Original Deadline" to "Action Plan Submission Deadline"
-- 3. Renamed "Days Overdue" to "Action Plan Submission Overdue"

UPDATE EmailTemplates 
SET html_body = '<div style="font-family: Segoe UI, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
...
available_placeholders = '["recipientName","storeName","documentNumber","auditDate","reportSentDate","deadline","daysOverdue","storeManagerName","storeManagerEmail","actionPlanUrl","dashboardUrl"]'
WHERE template_key = 'action_plan_escalation';

-- Also update the Action Plan Overdue template (sent to Store Manager)
UPDATE EmailTemplates 
SET html_body = '<div style="font-family: Segoe UI, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #dc2626 0%, #b91c1c 100%); padding: 20px; border-radius: 12px 12px 0 0; text-align: center;">
...
available_placeholders = '["recipientName","storeName","documentNumber","auditDate","reportSentDate","deadline","daysOverdue","actionPlanUrl","dashboardUrl"]'
WHERE template_key = 'action_plan_overdue';

-- Also update the Action Plan Reminder template
UPDATE EmailTemplates 
SET html_body = '<div style="font-family: Segoe UI, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
...
available_placeholders = '["recipientName","storeName","documentNumber","auditDate","reportSentDate","deadline","daysRemaining","actionPlanUrl","dashboardUrl"]'
